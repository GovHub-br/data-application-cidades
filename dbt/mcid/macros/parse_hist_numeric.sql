    ...
    try_cast(
        case
            ...
            when
                ...
                in ('', 'None', 'nan', 'NaN', 'null', 'NULL')
            then null
            when
                cast({{ col }} as varchar) like '%,%'
                and cast({{ col }} as varchar) like '%.%'
    ...
    date_trunc(
        ...
        try_cast(
            coalesce(
                ...
                try_strptime(
                    regexp_extract({{ fn }}, '[0-3]\d(0[1-9]|1[0-2])(20[0-2]\d)', 0),
                    '%d%m%Y'
                ),
                try_strptime(
                    nullif(
                        regexp_extract(
                            regexp_replace(
                                {{ fn }}, '(20[0-2]\d)[_-](0[1-9]|1[0-2])', '\1\2'
                            ),
                            '20[0-2]\d(?:0[1-9]|1[0-2])',
                            0
                        ),
                        ''
                    ),
