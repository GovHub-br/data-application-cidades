...
-- Bronze: Dados Prioritários SNH — snapshot corrente (frente FDS / Entidades)
-- Fonte: mcmv_staging.dados_prioritarios_disponibilizados_snh_empreendimentos
-- (snapshot único 30/09/2025; NÃO é a série mensal).
-- Recorte: upper(modalidade) = 'ENTIDADES'. Chave:
-- normalize_apf(codigo_da_operacao_no_agente_financeiro). ~247/335 casam com silver_fds.
with
    snh_raw as (
        select
        ...
        where upper(trim(modalidade)) = 'ENTIDADES'
    )
