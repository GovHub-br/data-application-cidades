...
with
    int059_raw as (
        select
            ...
            {{ normalize_apf('nu_apf_nao_obra') }} as apf_nao_obra,
            ...
            {{ parse_hist_numeric('vlr_operacao') }} as vr_operacao,
            {{ parse_hist_numeric('vr_investimento') }} as vr_investimento,
            {{ parse_hist_numeric('vr_liberado') }} as vr_liberado,
            {{ parse_hist_numeric('vr_liberado_sisfin') }} as vr_liberado_sisfin,
            ...
            {{ parse_date_br('dt_termino_obra') }} as dt_termino_obra,
            {{ parse_date_br('dt_legalizacao') }} as dt_legalizacao,
            {{ parse_date_br('dt_maxima_liberacao') }} as dt_maxima_liberacao,
            {{ parse_date_br('dt_ultima_entrega') }} as dt_ultima_entrega,
        ...
        from {{ source("mcmv_staging", "int_empreendimentos_int_059_fds_caixa_pj") }}
    )
