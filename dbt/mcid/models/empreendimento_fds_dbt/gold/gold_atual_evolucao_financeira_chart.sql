...
with
    evolucao_mensal as (select * from {{ ref("silver_fds_evolucao_financeira") }}),
...
select
    ...
    strftime(m.mes, '%Y-%m-%d') as mes,

    -- Agrupamentos temporais para o Superset
    strftime(date_trunc('year', m.mes), '%Y-%m-%d') as ano,
    strftime(date_trunc('quarter', m.mes), '%Y-%m-%d') as trimestre,
...
left join ficha f on m.apf = f.apf
