...
-- Prata: Empreendimento FDS — Visão unificada
...
with
    ...
    obra as (select * from {{ ref("prata_fds_obra_mensal") }}),
    ...
    int059 as (
        ...
        from
            (
                ...
                from {{ ref("prata_fds_int_059_caixa_pj") }}
                where selecao_pmcmv_e = 'NOVO PMCMV-E'
            ) t
        where rn = 1
    ),

    trabalho_social as (select * from {{ ref("prata_fds_trabalho_social") }}),
    ...
    desembolso_acumulado as (
        ...
        from {{ ref("prata_fds_financeiro_mensal") }}
