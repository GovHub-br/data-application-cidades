...
-- Prata: Empreendimento FAR — visão unificada
-- Reúne dados cadastrais, contratuais e status físico-financeiro de cada APF.
with
    cadastro as (select * from {{ ref("prata_far_cadastro_pj") }}),
    ...
    consolidado as (
        ...
        from
            (
                ...
                from {{ ref("prata_far_consolidado") }}
                where id_proposta is not null
            ) t
        where rn = 1
    ),

    obra as (select * from {{ ref("prata_far_obra_mensal") }}),

    caixa as (select * from {{ ref("prata_far_dados_prioritarios_caixa") }})
...
select
    ...
    case
        when coalesce(c.qt_uh, 0) > 0 then c.vr_total_investimento / c.qt_uh else 0.0
    end as valor_por_uh,
