...
-- Prata: Dados Prioritários CAIXA — snapshot consolidado da CAIXA
-- Fonte: bronze.bronze_sftp_snh_pmcmv_dados_prioritarios_af_caixa (a prata filtra FAR)
...
with
    prioritarios_raw as (
        select
            ...
            -- Datas
            case
                when data_de_contratacao is null or trim(data_de_contratacao) = ''
                then null
            ...
            case
                when
                    data_da_previsao_da_entrega is null
                    or trim(data_da_previsao_da_entrega) = ''
                then null
            ...
            -- Desembolso do ano
            {{ parse_financial_value("valor_desembolsado_do_ano_de_referencia") }}
