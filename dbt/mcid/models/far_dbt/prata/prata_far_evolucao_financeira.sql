...
-- Prata: Evolução Financeira — série temporal de desembolsos por empreendimento
...
with
    financeiro as (select * from {{ ref("prata_far_financeiro_mensal") }}),

    empreendimento as (select * from {{ ref("prata_far_empreendimento") }}),
