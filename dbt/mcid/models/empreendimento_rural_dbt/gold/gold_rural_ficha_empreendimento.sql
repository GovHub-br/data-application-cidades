...
with silver as (select * from {{ ref("silver_rural_empreendimento") }})
...
select
    ...
    eo_cnpj as cnpj_eo,
    regexp_replace(
        eo_cnpj, '^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$', '\1.\2.\3/\4-\5'
    ) as cnpj_eo_formatado,
    ...
    case
        co_modalidade
        when 1
        then 'Aquisição de Terreno e Projeto'
        when 2
        then 'Elaboração de Projeto'
        when 3
        then 'Produção de Unidades Novas'
        when 4
        then 'Requalificação'
    ...
    case
        when uf in ('AC', 'AP', 'AM', 'PA', 'RO', 'RR', 'TO')
        then 'Norte'
        when uf in ('AL', 'BA', 'CE', 'MA', 'PB', 'PE', 'PI', 'RN', 'SE')
        then 'Nordeste'
        when uf in ('DF', 'GO', 'MT', 'MS')
        then 'Centro-Oeste'
        when uf in ('ES', 'MG', 'RJ', 'SP')
        then 'Sudeste'
        when uf in ('PR', 'RS', 'SC')
        then 'Sul'
        else 'Não Informado'
    ...
    situacao_gefus,
    case
        when situacao_gefus = 'PARALISADA'
        then 'Paralisada'
        when situacao_gefus = 'NAO_INICIADA_CLAUSULA_SUSPENSIVA'
        then 'Cláusula Suspensiva'
        when percentual_execucao_fisica = 0
        then 'Não Iniciada'
        when percentual_execucao_fisica = 100
        then 'Concluída'
        else 'Em Andamento'
    end as semaforo_alerta,

    case
        when dt_paralisacao is not null then 'Sim' else 'Não'
    ...
    case
        when divergencia_fisico_financeira > 5
    ...
    case
        when pct_entrega = 100
        then 'Totalmente Entregue'
        when pct_entrega > 0
