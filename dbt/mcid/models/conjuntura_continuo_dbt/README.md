# conjuntura_continuo_dbt

Modelo dbt do **boletim de conjuntura contínuo** (dashboard atualizado
semanalmente, sempre mostrando o exercício atual). Roda no adapter
`dbt-postgres`, mas as fontes vêm do data lake via **pg_duckdb**.

## Arquitetura (pipeline por dado)

```
Etapa 01 (RAW)     CSV / TXT / XLSX / API-JSON  -> raw/<fonte>/<dado>.<ext>      (MinIO)
Etapa 02 (STAGING) TODOS convertidos p/ parquet -> staging/<fonte>/<dado>.parquet (MinIO, TIPADO)
Etapa 03 (SILVER)  pg_duckdb read_parquet        -> Postgres (este projeto dbt)
Etapa 04 (GOLD)    1 modelo por gráfico/card     -> Postgres, schema conjuntura_continuo_mart (Superset)
```

- **Etapa 02** é implementada pelo Template Method em
  `plugins/ingestor_lake.py` (manuais) e, para o IBGE, inline no
  `ibge_ingest_dag` (`gera_parquet_tipado`).
- **Não há camada bronze** aqui: o parquet de staging já sai tipado, então a
  `silver_*` é `select * from read_parquet('s3://...')` (passthrough) —
  exceto os dados manuais, cuja silver aponta para `manual_conjuntura.*`.
- **Etapa 04 (gold)**: 1 modelo por gráfico/card do boletim, no schema
