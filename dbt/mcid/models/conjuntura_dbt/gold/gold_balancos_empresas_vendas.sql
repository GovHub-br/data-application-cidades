...
with
    ...
    acum_26 as (
        select nome_empresa, sum(vendas) as vendas_12m_26
        from {{ ref("silver_balancos_empresas") }}
        where
    ...
    acum_25 as (
        select nome_empresa, sum(vendas) as vendas_12m_25
        from {{ ref("silver_balancos_empresas") }}
        where
    ...
    acum_24 as (
        select nome_empresa, sum(vendas) as vendas_12m_24
        from {{ ref("silver_balancos_empresas") }}
        where
...
select
    atual.nome_empresa,
    round(((atual.vendas::numeric / nullif(q4.vendas, 0)) - 1) * 100, 0) as variacao_4t25,
    round(
        ((atual.vendas::numeric / nullif(t25.vendas, 0)) - 1) * 100, 0
    ) as variacao_1t25,
    round(
        ((a26.vendas_12m_26::numeric / nullif(a25.vendas_12m_25, 0)) - 1) * 100, 0
    ) as variacao_12m_26_25,
    round(
        ((a25.vendas_12m_25::numeric / nullif(a24.vendas_12m_24, 0)) - 1) * 100, 0
