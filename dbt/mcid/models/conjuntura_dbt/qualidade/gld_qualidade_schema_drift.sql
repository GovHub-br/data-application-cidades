...
-- `gld_qualidade_schema`, este model devolve vazio — é o esperado, não é
-- erro. A partir do segundo dia de execução ele passa a ter conteúdo.

with retratos as (
    select distinct visto_em from {{ ref('gld_qualidade_schema') }}
),
...
agora as (
    select s.* from {{ ref('gld_qualidade_schema') }} s, marcos m
