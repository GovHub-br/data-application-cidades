...
-- Ouro do conjuntura: N° de UH por condição de uso — FGTS-PF e
-- SBPE Aquisição (novos x usados). Página 4.
--
-- Lado FGTS-PF é PARCIALMENTE automatizado (2026-08-29): sistema GEAVO da
-- Caixa, via prata_conjuntura_geavo_fgts_pf. Esta Prata contém o recorte
...
-- automatizada (`prata_conjuntura_abecip_financiamentos`), que tem a série
...
with fgts_pf as (
    ...
    from {{ ref('prata_conjuntura_geavo_fgts_pf') }}
    group by ano, mes
),
...
sbpe_split as (
    ...
    from {{ ref('prata_conjuntura_manual_mensais') }}
...
sbpe as (
    ...
    from {{ ref('prata_conjuntura_abecip_financiamentos') }} a
