...
-- Ouro do conjuntura: Produção Física (PIM-PF) e Vendas no varejo de
...
with pim as (
    ...
    from {{ ref('prata_conjuntura_ibge_pim_pf_brasil') }}
    group by periodo
),
...
pmc as (
    ...
    from {{ ref('prata_conjuntura_ibge_pmc_construcao') }}
    ...
    -- erro de digitação, porque 56732 não existia na prata. Estava errado:
    -- 56732 é o código certo, e some da prata justamente porque vem de outro
