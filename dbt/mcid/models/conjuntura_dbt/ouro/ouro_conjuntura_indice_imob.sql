...
-- Ouro do conjuntura: Índice IMOB (IMOB.SA via Alpha Vantage) —
...
-- prata_conjuntura_infomoney_imob.sql): o parquet da lake só tinha ~100
...
-- acumulado, senão esse ouro volta a ficar "curto" com o tempo).
--
-- Fonte única: a série automatizada. O coalesce com a planilha manual foi
-- removido em 2026-08-31 — a automatizada cobre 100% dos meses do ouro, então
...
with fechamentos_mensais as (
    ...
    from {{ ref('prata_conjuntura_infomoney_imob') }}
