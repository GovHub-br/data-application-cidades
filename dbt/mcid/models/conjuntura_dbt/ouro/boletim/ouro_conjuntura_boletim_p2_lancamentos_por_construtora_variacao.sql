...
-- Boletim de Conjuntura, página 2: Lançamentos por construtora (variação %)
...
edicoes as (
    ...
    serie as (
        select empresa, (right(periodo, 4)::int * 4 + left(periodo, 1)::int) as k,
               ...
               (case when btrim(var_lancamentos_tri_anterior::text, E' \t\r\n\u00a0') ~ '^-?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$' then btrim(var_lancamentos_tri_anterior::text, E' \t\r\n\u00a0')::numeric end)            as var_tri,
               (case when btrim(var_lancamentos_mesmo_tri_ano_anterior::text, E' \t\r\n\u00a0') ~ '^-?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?$' then btrim(var_lancamentos_mesmo_tri_ano_anterior::text, E' \t\r\n\u00a0')::numeric end)  as var_ano
        from {{ ref('ouro_conjuntura_balancos_empresas') }}
    )
