...
-- Ouro do conjuntura: OGU — dotação, empenho, pagamento, restos a
...
with acoes_boletim as (
    ...
    select *
    from {{ ref('prata_conjuntura_siafi_dotacao_execucao') }}
...
parsed as (
    ...
    select
        ...
        sum(despesas_empenhadas)      as empenho,
        sum(despesas_pagas)           as pagamento,
        sum(restos_a_pagar_inscritos)  as rap_inscrito,
        sum(restos_a_pagar_pagos)      as pag_rap,
        max(dt_ingest)::date                                                          as dt_referencia_extracao
    from acoes_boletim
