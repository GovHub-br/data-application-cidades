...
with
    cadastro as (
        select * from {{ ref("bronze_far_cadastro_pj") }}
    ),
    ...
    consolidado as (
        select * from (
            ...
            from {{ ref("bronze_far_consolidado") }}
            where id_proposta is not null
        ) t where rn = 1
    ),

    obra as (
        select * from {{ ref("bronze_far_obra_mensal") }}
    ),

    caixa as (
        select * from {{ ref("bronze_far_dados_prioritarios_caixa") }}
    ),

    -- SNH: snapshot corrente (30/09/2025). Enriquecimento aditivo — 1:1 por APF,
    -- ~813/822 casam. Não substitui nenhuma coluna existente (contrato só cresce).
    snh as (
        select * from {{ ref("bronze_far_dados_prioritarios_snh") }}
    )
...
select
    ...
    -- 5. Enriquecimento SNH (snapshot 30/09/2025) — colunas novas, aditivas
