...
select
    md5(concat_ws('|', 'mcmv-serie-temporal', linha_historica, ano::text, snapshot_date::text)) as id_historico_snapshot,
    md5(concat_ws('|', 'mcmv-serie-temporal', linha_historica, ano::text)) as id_negocio_historico,
    ...
    snapshot_date,
    md5(concat_ws('|', ano::text, linha_historica, quantidade_uh::text, fonte_primaria)) as hash_linha,
    ...
    'Piloto issue #118: serie historica anual usada para consultas temporais, backtest do relogio e validacao de reprocessamento.'::text as observacao_historico
from serie_longa
