...
fds_caixa as (  -- INT059
    select
        ...
        dt_ingest
    from {{ sftp }}
    where fonte_interface = 'INT059_MinisterioCidades_FDS_CAIXA_EMPREENDIMENTOS'
...
snh_entidades as (
    select
        ...
        dt_ingest
    from {{ snh }}
    where upper(nullif(trim(modalidade::text), '')) = 'ENTIDADES'
      and nullif(trim(apf::text), '') is not null
),

unioned as (
    select * from fds_caixa
    union all
    select * from snh_entidades
),
...
select
    ...
    quantidade_uh,
    coalesce(quantidade_uh_entregues, quantidade_uh_entregues_grao) as quantidade_uh_entregues,
    valor_contratado,
    valor_desembolsado,
