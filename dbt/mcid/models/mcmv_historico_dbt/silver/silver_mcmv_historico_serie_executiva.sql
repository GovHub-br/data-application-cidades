...
mapeado as (
    select
        ...
        case
            when lower(source_file) like 'caixa%' or lower(source_file) like '%_caixa%' then 'CAIXA'
            when lower(source_file) like 'bb%' or lower(source_file) like '%_bb%' then 'Banco do Brasil'
        ...
        {{ coalesce_present(bronze, ['municipio','vnome_municipio']) }} as municipio_raw,
        {{ coalesce_present(bronze, ['faixa','cfaixa','num_faixa','faixa_divisao']) }} as faixa_raw,
        {{ coalesce_present(bronze, [
        ...
        {{ coalesce_present(bronze, ['vnome_construtora','construtora','nom_proponente','empresa']) }} as responsavel_nome,
        {{ coalesce_present(bronze, ['inumero_cnpj','cnpj','cod_cnpj_proponente']) }} as responsavel_id,
        ...
        {{ coalesce_present(bronze, ['uh_em_obras','unidades_em_obras','iqde_uh_em_obras']) }} as uh_em_obras_raw,
        {{ coalesce_present(bronze, ['uh_comercializadas','comercializadas','qtd_comercializadas']) }} as uh_comercializadas_raw,
        ...
        {{ coalesce_present(bronze, ['valor_total_liberado','mvalor_desembolso']) }} as valor_liberado_raw,
        {{ coalesce_present(bronze, [
        ...
        {{ coalesce_present(bronze, ['data_contratacao','dat_contratacao','data_da_contratacao_bb']) }} as dt_contratacao_raw,
        {{ coalesce_present(bronze, ['data_conclusao','dat_entregue','entrega_do_empreendimento']) }} as dt_entrega_raw,
        {{ coalesce_present(bronze, [
...
tipado as (
    select
        ...
        regexp_replace(nullif(trim(cast(codigo_ibge_raw as varchar)), ''), '\D', '', 'g') as codigo_ibge_municipio,
        ...
        {{ parse_hist_double('pct_execucao_fisica_raw') }} as percentual_execucao_fisica,
...
classificado as (
    select
        *,
        case
            when coalesce(subsidio_ogu, 0) > 0 and coalesce(subsidio_ogu, 0) >= coalesce(subsidio_fgts, 0)
                then 'OGU/Subsidiado'
...
dedup as (
    select
        *,
        row_number() over (
            partition by fonte_familia, coalesce(chave_natural, hash_linha), dt_referencia
