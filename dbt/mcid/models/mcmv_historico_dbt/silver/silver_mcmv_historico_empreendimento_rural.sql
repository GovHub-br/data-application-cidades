...
rural_bb as (  -- INT057
    select
        ...
        {{ parse_hist_double('pc_execucao_fisica_obra') }} as percentual_execucao_fisica,
        ...
        dt_referencia,
        {{ parse_hist_date('coalesce(idt_movimento, dt_movimento)') }} as dt_movimento,
        ...
        dt_ingest
    from {{ sftp }}
    where fonte_interface = 'INT057_MinisterioCidades_PNHR_BB_EMPREENDIMENTOS'
...
rural_caixa as (  -- INT065
    select
        ...
        dt_ingest
    from {{ sftp }}
    where fonte_interface = 'INT065_MinisterioCidades_PNHR_CAIXA_EMPREENDIMENTOS'
...
snh_rural as (
    select
        ...
        dt_ingest
    from {{ snh }}
    where upper(nullif(trim(modalidade::text), '')) = 'RURAL'
