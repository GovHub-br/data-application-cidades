...
{{ code | lower }} as (
    ...
    from {{ read_minio_staging_parquet_series('sftp/fabrica/GEFUS/**/' ~ code ~ '_*.parquet') }}
    where regexp_matches(filename, '_\d{8}\.parquet$')
...
, unido as (
    {% for code, fonte in interfaces %}
    select * from {{ code | lower }}
    {{ "union all by name" if not loop.last }}
    {% endfor %}
)
