...
select
    *,
    ...
    coalesce(try_cast(dt_entrega as date), try_cast(dt_ass_doc as date)) as dt_evento,
    {{ parse_hist_bigint('coalesce(qt_uh_entregues, numero_de_unidades_entregues)') }}
    as qt_uh_entregues_evento,
    md5(
        concat_ws(
            '|',
            ...
            coalesce(
                cast(qt_uh_entregues as varchar),
                cast(numero_de_unidades_entregues as varchar),
                ''
            ),
