...
| # | achado | estado |
|---|---|---|
| G1 | `ouro_reloginho_indicadores` citava `silver_reloginho_snh_apf_mes` | corrigido → `prata_dhist_snh_apf_mes` |
...
| G3 | `ouro_reloginho_indicadores_gargalo_desempenho.frente` = {FAR, FDS} | description declara `FDS` ≡ `Entidades` e por que a coluna se chama `frente` |
| G4 | reloginho `uh_contratadas`/`uh_entregues`/`uh_vigentes` | nota D3 (conceito canônico `quantidade_uh*`) em silver + 4 golds |
| G5 | `ouro_reloginho_indicadores_gargalo_desempenho.dt_calculo` | description declara equivalência a `dt_gold` |
| G6 | `status_prazo` / `status_ritmo` vs `situacao_canonica` | domínio e eixo de cada um documentados |
| G7 | `ouro_reloginho_resumo_gargalo_desempenho_dashboard` sem unidade/escala | unidade (UH / R$ / 0-100 / dias) e grão por `nivel_agregacao` em cada coluna; +15 colunas físicas antes não documentadas |
| G8 | "UH entregues" em `ouro_dhist_serie_mensal` vs `ouro_reloginho_indicadores` | nota de `natureza_serie` / não-soma alinhada nas duas |
...
| G11 | `prata_dhist_snh_apf_mes` `ref()` dinâmico | `depends_on` lista as 2 bronzes SNH + 2 seeds — OK |
...
| G15 | endereço/CNPJ em `ouro_dhist_snapshot_empreendimento_atual` | descritas com nota explícita "empreendimento/entidade, não pessoa física"; decisão de publicar registrada no `design.md` (OQ5) |
...
| G18 | `ref()` cross-schema `prata_dhist_entrega_apf` → `reloginho` | verificado no `depends_on` — OK |
