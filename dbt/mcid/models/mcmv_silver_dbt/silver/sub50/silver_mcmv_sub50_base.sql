...
{% if apresentadas_relation is not none or selecionadas_relation is not none %}
    ...
    {% if apresentadas_relation is not none %}
        with
            propostas_apresentadas as (
                select
                    md5(
                        concat_ws('|', 'sub50-apresentada', numero_da_proposta::text)
                    ) as id_silver_frente,
                    ...
                    'raw/novo_mcmv_fnhis_sub_50_propostas_apresentadas.csv'::text
                    ...
                    coalesce(situacao_da_proposta, justificativa_nao_enquadramento)::text
                    ...
                    'Fonte FNHIS/SUB50 de propostas apresentadas localizada no MinIO e materializada em __dados_brutos; grao de proposta, nao APF.'
    ...
    {% if selecionadas_relation is not none %}
        ...
        {% endif %}
            propostas_selecionadas as (
                select
                    md5(
                        concat_ws('|', 'sub50-selecionada', num_proposta::text)
                    ) as id_silver_frente,
                    'Minha Casa Minha Vida'::text as programa,
                    'SUB50'::text as frente_mcmv,
                    'Subsidiada'::text as grupo_linha,
                    ...
                    'raw/novo_mcmv_fnhis_sub_50_propostas_selecionadas.csv'::text
                    as fonte_minio_staging,
                    null::text as apf,
                    ...
                    null::integer as quantidade_uh_entregues,
                    {{ parse_financial_value('coalesce(valor_de_repasse::text, vl_repasse_proposta::text, valor_empenhado_acumulado::text)') }}
                    as valor_contratado,
                    {{ parse_financial_value('coalesce(valor_empenhado_acumulado::text, vl_empenhado_pre_convenio::text)') }}
                    ...
                    null::date as dt_entrega,
                    coalesce(
                        {{ target.schema }}.parse_date_br(data_assinatura::text),
                        {{ target.schema }}.parse_date_br(data_consulta::text)
                    ) as dt_ultima_atualizacao,
                    'Fonte FNHIS/SUB50 de propostas selecionadas localizada no MinIO e materializada em __dados_brutos; grao de proposta selecionada.'
                    ::text as observacao_silver,
                    current_timestamp as dt_silver
