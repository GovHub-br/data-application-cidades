...
class ClienteNovoCaged(ClienteBase):
    ...
    def _parse_response(
        ...
        try:
            ...
            # O valor já chega como número nativo do JSON do Power BI (não texto
            # da fonte) — aqui só evita reformatar (ex.: int virando "1.0").
            valores = [v if v is None else str(v) for v in raw_values[:5]]
            return {
                "ano": ano,
                "mes": MAP_MESES.get(mes.lower(), mes),
                "admitidos": valores[0],
                "desligados": valores[1],
                "saldo": valores[2],
                "estoque": valores[3],
                "variacao": valores[4],
            }
