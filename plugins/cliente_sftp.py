...
class ClienteSftp:
    ...
    def conectar(self) -> paramiko.SFTPClient:
        ...
        sftp: Optional[paramiko.SFTPClient] = paramiko.SFTPClient.from_transport(
            transport
        )
        if sftp is None:
            transport.close()
        ...
        logging.info(f"[cliente_sftp.py] Conectado a {self.user}@{self.host}:{self.port}")
        return sftp
    ...
    def baixar_para_tempfile(self, caminho: str, tmpdir: Optional[str] = None) -> str:
