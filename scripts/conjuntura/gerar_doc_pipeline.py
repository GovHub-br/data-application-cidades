...
def dominio_de(chave: str, modelos: dict, manifest: dict, _memo=None) -> str:
    ...
    O nome não serve: `ouro_conjuntura_sinapi` não diz "ibge" em lugar nenhum,
...
def carregar_contagens() -> dict[str, int]:
    """Nº de linhas por model, lido de `ouro_conjuntura_qualidade_inventario`.
    ...
    try:
        ...
        r = subprocess.run(
            [
                ...
                "show",
                "--inline",
                "select model, linhas from conjuntura.ouro_conjuntura_qualidade_inventario",
                "--output",
                "json",
                "--limit",
                "300",
