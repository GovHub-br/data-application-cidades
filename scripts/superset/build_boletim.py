...
def pagina_01() -> list[Quadro]:
    ...
    serie as (
        ...
        from {MART}.ouro_conjuntura_pib_construcao_civil_pct
    ),
    m as (
        select e.edicao, e.k,
...
def pagina_02() -> list[Quadro]:
    ...
    serie as (
        ...
        from {MART}.ouro_conjuntura_balancos_empresas
    )
    ...
    soma as (
        ...
        from {MART}.ouro_conjuntura_balancos_empresas group by 1
    ),
    tot as (
        select (right(periodo, 4)::int * 4 + left(periodo, 1)::int) as k,
        ...
        from {MART}.ouro_conjuntura_balancos_empresas_totais
    )
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_financiamentos_imobiliarios_pf_pj
    ),
    ...
    serie as (
        ...
        from {MART}.ouro_conjuntura_financiamentos_habitacionais
    )
...
def pagina_03() -> list[Quadro]:
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_empregos_caged
    ),
    ...
    base as (
        ...
        from {MART}.ouro_conjuntura_pnad_ocupados o
        join {MART}.ouro_conjuntura_pnad_rendimento r on r.periodo = o.periodo
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_producao_fisica
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
    ...
    bruto as (
        ...
        from ref r
        join {MART}.ouro_conjuntura_financiamentos_instituicao g
...
def pagina_04() -> list[Quadro]:
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_credito_pib
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
    select r.edicao, x.rotulo as periodo,
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_uh_condicao_uso
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
...
mes as (
    ...
    from {SILVER}.prata_conjuntura_abecip_financiamentos
),
ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
...
""",
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_saldo_poupanca
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
    select r.edicao, x.rotulo as periodo,
    ...
    mes as (
        ...
        from {MART}.ouro_conjuntura_financiamento_pf_faixa
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes),
    f as (
...
def pagina_06() -> list[Quadro]:
    ...
    return [
        Quadro(
            ...
            #: `ouro_conjuntura_ogu` guarda uma extração pontual, e o model do
    ...
    from {MART}.ouro_conjuntura_boletim_p6_ogu
""",
        ),
        Quadro(
            pagina=6,
    ...
    sin as (
        ...
        from {MART}.ouro_conjuntura_sinapi
    ),
    inc as (
        ...
        from {MART}.ouro_conjuntura_incc_m
    ),
    ref as (select edicao, ano_ed * 12 + tri_ed * 3 as m0 from edicoes)
    ...
    serie as (
        ...
        from {MART}.ouro_conjuntura_ticket_medio
    )
...
def pagina_07() -> list[Quadro]:
    ...
    icst as (
        ...
        from {MART}.ouro_conjuntura_icst
    )
...
#: Quadros cujo nome derivado do título passa dos 63 bytes que o Postgres
#: aceita como identificador. O prefixo de camada da arquitetura nova
#: (`ouro_conjuntura_boletim_p<N>_`) é 12 bytes mais longo que o antigo, e nestes
#: oito o slug do título não cabe mais. O nome curto é escrito à mão; nos
#: outros treze o derivado continua servindo.
RENOMEADOS = {
    "gld_boletim_p1_pib_construcao_civil_em_de_crescimento": "ouro_conjuntura_boletim_p1_pib_construcao_crescimento",
    "gld_boletim_p2_totais_das_empresas_levantadas_variacao": "ouro_conjuntura_boletim_p2_totais_empresas_variacao",
    "gld_boletim_p3_novos_financiamentos_imobiliarios_por_banco_": "ouro_conjuntura_boletim_p3_financiamentos_por_banco",
    "gld_boletim_p3_pnad_continua_ocupados_e_rendimento_medio_re": "ouro_conjuntura_boletim_p3_pnad_ocupados_rendimento",
    "gld_boletim_p3_producao_industrial_e_volume_de_vendas_varia": "ouro_conjuntura_boletim_p3_producao_e_vendas_variacao",
    "gld_boletim_p5_saldo_caderneta_de_poupanca_captacao_liquida": "ouro_conjuntura_boletim_p5_poupanca_captacao_liquida",
    "gld_boletim_p5_sbpe_construcao_unidades_e_valor_acum_no_tri": "ouro_conjuntura_boletim_p5_sbpe_construcao_no_tri",
    "gld_boletim_p6_ticket_medio_das_unidades_lancadas_vs_incc": "ouro_conjuntura_boletim_p6_ticket_medio_vs_incc",
}
...
def nome_dataset(q: Quadro) -> str:
    ...
    derivado = f"gld_boletim_p{q.pagina}_{base}"
    return RENOMEADOS.get(derivado, f"ouro_conjuntura_boletim_p{q.pagina}_{base}")
