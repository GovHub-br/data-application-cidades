...
_DEFAULT_SCHEMA_SFTP = "sftp"
_DEFAULT_SCHEMA_SFTP_TARGET = "sftp_tratado"
...
def get_schema_sftp() -> str:
    """Return the SFTP schema name from environment or the default.

    Reads ``DB_SCHEMA_SFTP``. When the variable is not set, returns
    ``"sftp"`` (the standard GEFUS schema).

    Returns
    -------
    str
        Schema name for the SFTP/GEFUS tables.
    """
    schema = os.environ.get("DB_SCHEMA_SFTP")
    if schema is None:
        return _DEFAULT_SCHEMA_SFTP
    return schema


def get_schema_sftp_target() -> str:
    """Return the SFTP target schema name from environment or the default.

    Reads ``DB_SCHEMA_SFTP_TARGET``. When the variable is not set, emits a
    warning and returns ``"sftp_tratado"``.

    Returns
    -------
    str
        Schema name to write SFTP treated tables to.
    """
    schema = os.environ.get("DB_SCHEMA_SFTP_TARGET")
    if schema is None:
        logger.warning(
            "DB_SCHEMA_SFTP_TARGET não definida. Usando padrão: '%s'.",
            _DEFAULT_SCHEMA_SFTP_TARGET,
        )
        return _DEFAULT_SCHEMA_SFTP_TARGET
    return schema
