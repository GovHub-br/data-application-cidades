...
def test_r2_tab_arquivos_dados_com_pipe_nao_descarta() -> None:
    ...
    assert (
        r2_dados_sem_utilidade("bb_2013_06_junho_pmcmv_18062013_tab_arquivos_dados", df)
        is False
    )
...
def test_pipe_com_uma_coluna_nao_eh_vazia() -> None:
    """Tabela pipe com 1 coluna e tamanho pequeno não deve ser vazia."""
    df = pd.DataFrame({"registro": ["123|abc|2013-01-01"] * 5})
    prof = profile_estrutural(
        df, file_size=500
    )  # < 5KB → acionaria R1 se R3 não primeiro
    formacao, conf, _notes = classificar_formacao("test_pipe", df, prof)
    assert formacao == SEPARADOR_PIPE
    assert conf == "high"


def test_pipe_sem_dados_vazio() -> None:
    """Tabela com 0 linhas de dados e coluna 'registro' sem pipe → vazia."""
    df = pd.DataFrame({"registro": pd.Series(dtype="object")})
    assert len(df) == 0
    assert r3_separador_pipe(df) is False
    prof = profile_estrutural(df, file_size=10)
    formacao, conf, _notes = classificar_formacao("test_empty", df, prof)
    assert formacao == VAZIA
    assert conf == "high"


def test_r2_excecao_pipe_ainda_funciona() -> None:
    """R2 exceção (tab_arquivos_dados com pipe) continua retornando False."""
    df = pd.DataFrame({"registro": ["2023-01-01|123|resp|email@gov.br"] * 5})
    assert (
        r2_dados_sem_utilidade("bb_2013_06_junho_pmcmv_18062013_tab_arquivos_dados", df)
        is False
    )
    # Com file_size pequeno (<5KB), antes da correção retornaria vazia.
    # Agora R3 detecta pipe antes de R1.
    prof = profile_estrutural(df, file_size=500)
    formacao, conf, _notes = classificar_formacao(
        "bb_2013_06_junho_pmcmv_18062013_tab_arquivos_dados", df, prof
    )
    assert formacao == SEPARADOR_PIPE
