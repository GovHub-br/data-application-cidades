...
def sample_table(
    ...
    count = db.query(
        f"select count(*) as n_rows from {quote_ident(schema)}.{quote_ident(table_name)}"
    )
    sample = db.query(
...
def pipe_metrics(df: pd.DataFrame) -> tuple[int, int, pd.DataFrame]:
    ...
    pipe_cells = int(
        as_text.apply(lambda col: col.str.contains(r"\|", regex=True)).sum().sum()
    )
...
def main() -> None:
    ...
    for table_name in TARGETS:
        ...
        before = (
            current.loc[table_name]
            if table_name in current.index
        ...
        prints.append(
            {
                "table_name": table_name,
                "raw_sample_print": raw_sample.head(6).to_string(
                    index=False, max_cols=8, max_colwidth=58
                ),
                "expanded_sample_print": expanded.head(6).to_string(
                    index=False, max_cols=12, max_colwidth=32
                ),
            }
        )
        ...
        [
            {
                ...
                "tabelas_corrigidas": int(
                    corrections["pipe_cells_amostra"].gt(0).sum()
                ),
                "pipe_cells_total_amostra": int(
                    corrections["pipe_cells_amostra"].sum()
                ),
                "linhas_brutas_total_corrigidas": int(
                    corrections["n_rows_corrigido"].sum()
                ),
            }
        ]
    )
