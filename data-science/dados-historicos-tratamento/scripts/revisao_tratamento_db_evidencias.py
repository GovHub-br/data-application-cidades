...
def sample_table(db: Psql, schema: str, table_name: str, limit: int = 8) -> tuple[bool, str, list[str]]:
    exists_sql = f"""
        select exists (
...
def main() -> None:
    ...
    quality["missing_pct_num"] = pd.to_numeric(quality["missing_pct"], errors="coerce").fillna(0)
    ...
    ].sort_values(["institution", "report_date", "n_rows"], ascending=[True, True, False])
    ...
    for _, row in selected.iterrows():
        ...
        if str(row.get("status", "")) == "discarded" and raw_exists and "|" in raw_sample:
        ...
        [
            {
                ...
                "institution_unknown": int(quality["institution"].isin(["", "unknown"]).sum()),
