...
def build_audit() -> pd.DataFrame:
    ...
    inv = pd.read_csv(INVENTARIO, sep="\t", dtype=str).fillna("").rename(
        columns={
    ...
    audit["match_classificacao_inventario"] = (
        audit["classificacao_formacao"].eq(audit["inventario_formacao"])
    ...
    audit["flag_revisao"] = ""
    audit.loc[audit["classificacao_confidence"].eq("low"), "flag_revisao"] += "confidence_low;"
    audit.loc[~audit["match_classificacao_inventario"], "flag_revisao"] += "classificacao_vs_inventario;"
    audit.loc[~audit["match_qualidade_inventario"], "flag_revisao"] += "qualidade_vs_inventario;"
            ...
            rows.append(
                {
                    ...
                    "modified": "",
                    "unchanged": "",
                    "same_size_false": sum(1 for item in data if item.get("same_size") is False),
                    "container_mapped": sum(1 for item in data if item.get("container_path")),
                }
            )
        else:
            rows.append(
...
def write_figures(audit: pd.DataFrame, corrections: pd.DataFrame, sftp: pd.DataFrame) -> None:
    ...
    ax.barh(counts.index.str.replace("separador_|", "separador pipe", regex=False), counts.values, color=colors)
    ax.set_title("Distribuicao revisada por categoria estrutural", fontsize=16, weight="bold")
    ...
    before_low = (corrections.get("confidence_antes", pd.Series(dtype=str)) == "low").sum()
    ...
    for i, value in enumerate([before_low, after_low]):
        ax.text(i, value + 0.05, str(int(value)), ha="center", fontsize=12, weight="bold")
    fig.tight_layout()
    ...
    if not diff_rows.empty:
        ...
        labels = diff_rows["artefato"].str.replace("diff_sftp_minio_", "", regex=False).str.replace(".json", "", regex=False)
        x = range(len(diff_rows))
        ax.bar([i - 0.25 for i in x], pd.to_numeric(diff_rows["added"]), width=0.25, label="added", color="#2563eb")
        ax.bar(x, pd.to_numeric(diff_rows["removed"]), width=0.25, label="removed", color="#dc2626")
        ax.bar([i + 0.25 for i in x], pd.to_numeric(diff_rows["modified"]), width=0.25, label="modified", color="#f59e0b")
        ...
        [
            {
                ...
                "sample_exists": (ROOT / "data" / "table_samples" / f"{table}.csv").exists(),
