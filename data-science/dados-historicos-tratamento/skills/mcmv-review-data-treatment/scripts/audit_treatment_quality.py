...
def read_csv_any(path: Path, **kwargs) -> pd.DataFrame:
    try:
        ...
        return pd.read_csv(path, dtype=str, keep_default_na=False, encoding="latin1", **kwargs).fillna("")
...
def inspect_treated(path: Path, table_name: str, sample_rows: int) -> tuple[dict[str, object], str]:
    try:
    ...
    empty_rows = int(text_df.apply(lambda row: all(v.strip() in ("", "-") for v in row), axis=1).sum())
    empty_cols = int(sum(text_df[c].map(lambda v: v.strip() in ("", "-")).all() for c in df.columns))
    duplicate_rows = int(df.duplicated().sum()) if len(df) else 0
    duplicate_cols = int(pd.Index(df.columns).duplicated().sum())
    unnamed_cols = int(sum(str(c).lower().startswith("unnamed") or str(c).strip() == "" for c in df.columns))
    mojibake_cells = int(text_df.apply(lambda s: s.str.contains(MOJIBAKE_RE, regex=True, na=False)).sum().sum())
    ...
    for col in id_cols[:12]:
        ...
        denom = max(len(raw), 1)
        null_rate = float(raw.map(lambda v: v.strip() in ("", "-", "nan", "None")).sum() / denom)
...
def build_sftp(root: Path) -> pd.DataFrame:
            ...
            rows.append(
                {
                    ...
                    "tipo": "lista",
                    "registros": len(data),
                    "same_size_false": sum(1 for item in data if isinstance(item, dict) and item.get("same_size") is False),
                    "container_mapped": sum(1 for item in data if isinstance(item, dict) and item.get("container_path")),
                }
            )
    ...
    columns = ["artefato", "tipo", "registros", "added", "removed", "modified", "unchanged", "same_size_false", "container_mapped", "erro"]
...
def main() -> None:
    ...
    parser.add_argument("--limit-tables", type=int, default=0, help="0 means all treated files found")
    args = parser.parse_args()

    root = Path(args.root).resolve()
    out = Path(args.output_dir).resolve() if args.output_dir else root / "docs" / "evidencias" / "revisao-tratamento-dados"
    out.mkdir(parents=True, exist_ok=True)

    quality_path = latest(root, ["*_qualidade_*.csv", "data/*_qualidade_*.csv", "data/treated_tables/_quality_report.csv"])
    dedup_path = latest(root, ["*_dedup_map_*.csv", "data/*_dedup_map_*.csv", "data/treated_tables/_dedup_map.csv"])
    class_path = latest(root, ["*_classificacao_*.csv", "data/*_classificacao_*.csv", "data/classificacao_formacao.csv"])
    ...
    if class_path:
        cls = read_csv_any(class_path).rename(columns={"formacao": "classificacao_formacao", "confidence": "classificacao_confidence"})
        audit = audit.merge(cls[[c for c in ["table_name", "classificacao_formacao", "classificacao_confidence"] if c in cls.columns]], on="table_name", how="left")

    if inventory_path.exists():
        inv = pd.read_csv(inventory_path, sep="\t", dtype=str, keep_default_na=False).fillna("")
        keep = [c for c in ["table_name", "n_linhas", "n_colunas", "status_tratamento", "perfil", "score_utilidade_preditiva"] if c in inv.columns]
        audit = audit.merge(inv[keep], on="table_name", how="left", suffixes=("", "_inventario"))

    if dedup_path:
        dedup = read_csv_any(dedup_path).rename(columns={"source_table": "table_name"})
        keep = [c for c in ["table_name", "canonical_table", "content_hash", "is_duplicate", "file_size"] if c in dedup.columns]
    ...
    audit["flag_revisao"] = ""
    audit.loc[audit.get("status", "").astype(str).eq("error"), "flag_revisao"] += "status_error;"
    audit.loc[audit.get("status", "").astype(str).eq("discarded"), "flag_revisao"] += "status_discarded;"
    audit.loc[pd.to_numeric(audit.get("n_rows", 0), errors="coerce").fillna(0).eq(0), "flag_revisao"] += "zero_rows;"
    audit.loc[pd.to_numeric(audit.get("n_cols", 0), errors="coerce").fillna(0).eq(0), "flag_revisao"] += "zero_cols;"
    audit.loc[pd.to_numeric(audit.get("missing_pct", 0), errors="coerce").fillna(0).gt(30), "flag_revisao"] += "missing_gt_30;"
    audit.loc[audit.get("institution", "").astype(str).isin(["", "unknown"]), "flag_revisao"] += "institution_unknown;"
    audit.loc[audit.get("report_date", "").astype(str).eq(""), "flag_revisao"] += "report_date_missing;"
    for col, flag in [("encoding_issues", "encoding_warning"), ("date_parse_errors", "date_parse_warning"), ("type_coercion_warnings", "type_warning")]:
        if col in audit.columns:
            audit.loc[pd.to_numeric(audit[col], errors="coerce").fillna(0).gt(0), "flag_revisao"] += f"{flag};"
    for col, flag in [("mojibake_cells", "mojibake_detected"), ("duplicate_cols", "duplicate_columns"), ("empty_cols", "empty_columns")]:
        if col in audit.columns:
            audit.loc[pd.to_numeric(audit[col], errors="coerce").fillna(0).gt(0), "flag_revisao"] += f"{flag};"
    ...
    if samples.empty:
        samples = pd.DataFrame([{"table_name": "", "sample_print": "Nenhum CSV tratado encontrado em data/treated_tables/."}])
            ...
            [
                {
                    ...
                    "duplicatas": int(dedup_raw.get("is_duplicate", pd.Series(dtype=str)).astype(str).str.lower().isin(["true", "1", "yes"]).sum()),
                    "hashes_unicos": int(dedup_raw.get("content_hash", pd.Series(dtype=str)).nunique()) if "content_hash" in dedup_raw.columns else "",
                }
            ]
        )
    ...
    summary = {
        ...
        "status_discarded": int((audit.get("status", "").astype(str) == "discarded").sum()),
    }
    ...
    (out / "summary_tratamento.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
