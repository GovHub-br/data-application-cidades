...
def table_html(df: pd.DataFrame, columns: list[str], limit: int = 20) -> str:
    ...
    for _, row in view.iterrows():
        body.append(
            "<tr>" + "".join(f"<td>{esc(row[c])}</td>" for c in available) + "</tr>"
        )
    return (
...
def main() -> None:
    ...
    sftp = pd.read_csv(
        evidence / "impacto_sftp_tratamento_pandas.csv", dtype=str
    ).fillna("")
    summary = json.loads(
        (evidence / "summary_tratamento.json").read_text(encoding="utf-8")
    )
    db_summary_path = evidence / "resumo_db_tratamento_pandas.csv"
    db_discord_path = evidence / "discordancias_tratamento_db_pandas.csv"
    db_summary = (
        pd.read_csv(db_summary_path, dtype=str).fillna("")
        if db_summary_path.exists()
        else pd.DataFrame()
    )
    db_discord = (
        pd.read_csv(db_discord_path, dtype=str).fillna("")
        if db_discord_path.exists()
        else pd.DataFrame()
    )
    use_db = not db_summary.empty and not db_discord.empty

    status_counts = (
        audit["status"].value_counts().to_dict() if "status" in audit.columns else {}
    )
    if use_db:
        ...
        discarded_count = int(
            summary.get("status_discarded", status_counts.get("discarded", 0))
        )
    ...
    if "error" in audit.columns:
        identifier_length_errors = int(
            audit["error"]
            .astype(str)
            .str.contains("exceeds maximum length", regex=False)
            .sum()
        )
    ...
    if "missing_pct" in missing_top.columns:
        missing_top["_missing_pct_num"] = pd.to_numeric(
            missing_top["missing_pct"], errors="coerce"
        ).fillna(0)
        missing_top = missing_top.sort_values("_missing_pct_num", ascending=False)
    if use_db:
        missing_top = db_discord[
            db_discord["motivo_discordancia"].eq("missing_alto")
        ].copy()
        missing_top["_missing_pct_num"] = pd.to_numeric(
            missing_top["missing_pct"], errors="coerce"
        ).fillna(0)
    ...
    if treated_files_found == 0:
        if use_db:
            missing_evidence.append(
                "data/treated_tables/ nao existe no repo, mas as amostras foram lidas diretamente do schema dados_historicos_formatados."
            )
        else:
            missing_evidence.append(
                "data/treated_tables/ nao existe nesta branch; nao foi possivel abrir CSV tratado linha a linha."
            )
    if len(sftp) == 0:
        missing_evidence.append(
            "diff_sftp_minio*.json nao foi encontrado nesta branch/worktree; impacto SFTP detalhado ficou pendente."
        )
    if (
        len(samples)
        and "sample_print" in samples.columns
        and "Nenhum CSV tratado" in str(samples.iloc[0]["sample_print"])
    ):
        missing_evidence.append(
            "amostras_tratadas_pandas.csv registra ausencia de amostras tratadas para prints reais."
        )
    if use_db:
    ...
    sample_print = (
        esc(samples.iloc[0]["sample_print"])
        if len(samples) and "sample_print" in samples.columns
        else "Sem amostras."
    )
    ...
    missing_evidence_html = (
        "".join(f"<li>{esc(item)}</li>" for item in missing_evidence)
        or "<li>Nenhuma ausencia critica de evidencia registrada.</li>"
    )
    ...
    recommendations_html = "".join(
        f"<li>{esc(item)}</li>" for item in recommendation_items
    )
    if use_db:
