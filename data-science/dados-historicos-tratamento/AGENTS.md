...
- Declared runtime deps: `duckdb`, `pandas`, `charset-normalizer`, `ftfy`, `matplotlib`, `sqlalchemy`, `psycopg2-binary`, `python-dotenv`. Dev: `mypy`, `pytest`, `ruff`.
...
- `src/` layout. The importable package is `classificacao` (not `src.classificacao`), with modules:
  ...
  - `db/reader.py` — DB read: `listar_tabelas()`, `ler_tabela()`, `ler_schema_colunas()`, `calcular_hash_db()`
  - `db/writer.py` — DB write: `criar_schema_target()`, `escrever_tabela()`, `escrever_classificacao()`, `escrever_qualidade()`, `escrever_dedup_map()`
- `main.py` at repo root is the entry point: classify → dedup → treat → quality report. With `--inventario`: also runs inventory, quality report, and timeline chart (`scripts/gerar_linha_do_tempo.py`). With `--db`: runs full pipeline against PostgreSQL (requires `.env` and pre-configured network namespace `protected`).
- O modo DB requer um network namespace Linux chamado `protected` pré-configurado com túnel OpenVPN e proxy `socat` TCP na porta 5432. O comando de execução é `sudo ip netns exec protected sudo -u $USER ./.venv/bin/python main.py --db [flags]`. A configuração do namespace é responsabilidade do operador.

## Data layout

- `data/table_samples/` — 753 CSV files (200 lines each), **tab-separated** (`sep="\t"`), first column is a row index (discard via `index_col=0`). README incorrectly calls this `data/sample_data`.
...
- `data/treated_tables/` — output for stage 3 (~440 treated CSVs + `_dedup_map.csv` + `_quality_report.csv`).
...
- **Classification reference:** `classificacao_formacao_revisado_autoritativo.md` — the authoritative source for category definitions (~17 categories with examples). This is what `comparar_referencia()` reads.
...
- In-progress changes: `openspec/changes/` (currently: `add-classificacao-formacao/`). Archived: `openspec/changes/archive/`. Synced specs: `openspec/specs/` (currently empty).

## Conventions

- Prioritize retrieval-led reasoning over pretrained-knowledge-led reasoning.
- Never apply a change without asking explicitly.
- Use the right specialist agent (@fixer for implementation, @oracle for architecture/code review, @designer for UI, etc.).
...
- README calls the samples folder `data/sample_data` — the actual name is `data/table_samples`.
...
- `openspec/specs/` is empty; synced specs haven't been generated yet.
...
- DB mode requires `.env` with valid credentials and a pre-configured network namespace `protected` (OpenVPN tunnel + socat proxy). Execution command: `sudo ip netns exec protected sudo -u $USER ./.venv/bin/python main.py --db [flags]`.
