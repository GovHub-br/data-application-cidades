...
estrutural. A árvore de decisão ``classificar_formacao`` orquestra R2→R3→R1→R3b→R5→R6→R7→R8 e
...
def r2_dados_sem_utilidade(table_name: str, df: pd.DataFrame | None = None) -> bool:
    ...
    if df is not None and any(
        p in table_name for p in PADROES_SEM_UTILIDADE_PIPE_EXCECAO
    ):
...
def classificar_formacao(
    ...
    """Orquestra R2→R3→R1→R3b→R5→R6→R7→R8 e retorna ``(formacao, confidence, notes)``.

    Árvore de decisão (D4):
        R2 → R3 → R1 → R3b → R5 → R6 → R7 → R8
    """
    ...
    # R2: dados sem utilidade (por nome) — tem prioridade sobre R1/R3
    if r2_dados_sem_utilidade(table_name, df):
        return (DADOS_SEM_UTILIDADE, "high", "")

    # R3: separador pipe (verificado antes de R1 — pipe prevalece sobre vazia)
    if r3_separador_pipe(df):
