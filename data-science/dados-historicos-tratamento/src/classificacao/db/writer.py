...
def comentar_tabela(
    ...
    """Adiciona um comentário PostgreSQL com metadados de proveniência.

    Serializa o dicionário *metadata* no formato ``key=val;key=val;...``
    e executa ``COMMENT ON TABLE``.

    Valores que contenham ``;`` ou ``=`` têm esses caracteres substituídos
    por ``,`` e ``:`` respectivamente. A string serializada é truncada em
    500 caracteres.
    ...
    engine : sqlalchemy.engine.Engine
        Database engine.
    schema : str
        Schema onde a tabela está.
    table_name : str
        Nome da tabela (já sanitizado).
    metadata : dict
        Dicionário com os metadados a comentar.
    """
    parts = []
    ...
    try:
        with engine.connect() as conn:
            conn.execute(
                text(f'COMMENT ON TABLE "{schema}"."{table_name}" IS :comment'),
                {"comment": comment},
            )
            conn.commit()
        logger.info(
            "Comentário adicionado em %s.%s (%d campos).",
            schema,
            table_name,
            len(metadata),
        )
        ...
        logger.warning(
            "Falha ao adicionar comentário em %s.%s: %s",
            schema,
            table_name,
            exc,
        )
