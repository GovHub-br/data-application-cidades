...
- **184** pares identificados por similaridade de colunas (Jaccard ≥ 0,5)
- Score médio: 0.72

| Método | Confiança | Qtd. Pares | Score Médio |
|--------|-----------|------------|-------------|
| Similaridade de Colunas (Jaccard) | Baixa | 184 | 0.72 |
...
| _202410_snh_pmcmv_dados_prioritarios_af_caixa | historico_recente_202410_snh_pmcmv_dados_prioritarios_af_cai... | 0.80 |
| _202412_snh_pmcmv_dados_prioritarios_af_caixa | historico_recente_202507_snh_pmcmv_dados_prioritarios_af_cai... | 0.81 |

### Stem Canônico (Confiança Media)

- **27** pares com stem canônico compatível e Jaccard ≥ 0,3
- Score médio: 0.42

| Método | Confiança | Qtd. Pares | Score Médio |
|--------|-----------|------------|-------------|
| Stem Canônico | Media | 27 | 0.42 |
...
| _202411_snh_pmcmv_dados_prioritarios_af_caixa_entregas | o_recente_202406_snh_pmcmv_dados_prioritarios_af_caixa_entre... | 0.42 |
| _202412_snh_pmcmv_dados_prioritarios_af_caixa_entregas | o_recente_202406_snh_pmcmv_dados_prioritarios_af_caixa_entre... | 0.42 |
| _202504_snh_pmcmv_dados_prioritarios_af_caixa_entregas | o_recente_202406_snh_pmcmv_dados_prioritarios_af_caixa_entre... | 0.42 |
...
|-------|--------|-------------|-------------|--------|
| nu_apf | `\bnu_apf\b` | 94 | 2 | ⚠️ requer validação |
...
- **gefus_***: 20 tabelas SFTP, 0 tabelas dump. Disponível apenas no SFTP — usar como fonte primária para esta família.

### Exemplos de JOIN

```sql
SELECT *
FROM sftp.tabela_a t1
INNER JOIN dados_historicos.tabela_b t2
    ON t1.agente_financeiro = t2.agente_financeiro
WHERE t1.agente_financeiro IS NOT NULL
  AND t2.agente_financeiro IS NOT NULL;
```
...
INNER JOIN dados_historicos.tabela_b t2
    ON t1.apf = t2.apf
WHERE t1.apf IS NOT NULL
  AND t2.apf IS NOT NULL;
```
...
FROM sftp.tabela_a t1
INNER JOIN dados_historicos.tabela_b t2
    ON t1.data_de_movimento = t2.data_de_movimento
WHERE t1.data_de_movimento IS NOT NULL
  AND t2.data_de_movimento IS NOT NULL;
```
...
FROM sftp.tabela_a t1
INNER JOIN dados_historicos.tabela_b t2
    ON t1.dt_ass_doc = t2.dt_ass_doc
WHERE t1.dt_ass_doc IS NOT NULL
  AND t2.dt_ass_doc IS NOT NULL;
```
...
FROM sftp.tabela_a t1
INNER JOIN dados_historicos.tabela_b t2
    ON t1.número_de_unidades_entregues = t2.número_de_unidades_entregues
WHERE t1.número_de_unidades_entregues IS NOT NULL
  AND t2.número_de_unidades_entregues IS NOT NULL;
```

### Limitações Conhecidas

- A análise é baseada exclusivamente em **metadados estruturais** (nomes de tabela, colunas e tipos), sem validação de conteúdo.
