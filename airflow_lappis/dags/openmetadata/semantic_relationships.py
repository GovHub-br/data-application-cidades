...
def _split_column_reference(reference: str) -> tuple[str, str, str]:
    ...
    if len(parts) != 3 or any(not part.strip() for part in parts):
        raise ValueError(
            "Referência de coluna inválida. Use schema.tabela.coluna: "
            f"{reference!r}"
        )
...
def load_semantic_catalog(catalog_path: str) -> dict[str, Any]:
    ...
    if len(table_keys) != int(scope["table_count"]):
        raise ValueError(
            "scope.table_count diverge da quantidade em tables_by_schema."
        )
    ...
    for group in catalog.get("search_groups", []):
        ...
        if counts and sum(int(value) for value in counts.values()) != int(
            ...
            raise ValueError(
                f"{group_id}: member_count diverge de members_by_schema."
            )
    ...
    for model in models:
        ...
        if f"{schema_name}.{table_name}" not in table_key_set:
            raise ValueError(
                f"Lógica de JOIN fora do escopo: {model['model']}"
            )
        if int(model["join_count"]) != len(model.get("predicates", [])):
            raise ValueError(
                f"join_count diverge dos predicados em {model['model']}."
            )
...
def validate_catalog_against_dbt(
    ...
    if column_count != int(catalog["scope"]["column_count"]):
        raise ValueError(
            "scope.column_count diverge das colunas documentadas no dbt."
        )
...
def _table_fqn(catalog: dict[str, Any], table_key: str) -> str:
    ...
    scope = catalog["scope"]
    return ".".join(
        (scope["service"], scope["database"], schema_name, table_name)
    )
...
def _relationships_for_table(
    ...
    for section_name, section_label, relationship in _iter_relationships(catalog):
        ...
        if table_key == source:
            result.append(
                (section_label, relationship, "origem", relationship["target"])
            )
...
def _patch_custom_properties_preserving_extension(
    ...
    response = metadata.client.patch(
        path=(
            f"{metadata.get_suffix(entity)}/"
            f"{str(_plain_value(table.id))}"
        ),
        data=json.dumps(operations, ensure_ascii=False),
    )
    if not response:
        raise RuntimeError(
            "OpenMetadata não retornou a tabela após o PATCH semântico."
        )
    return response
...
def sync_semantic_relationships(
    ...
    # Imports tardios: o módulo também é testado fora do virtualenv isolado.
    ...
    if missing_tables:
        raise RuntimeError(
            "Tabelas do catálogo ausentes no OpenMetadata: "
            + ", ".join(missing_tables)
        )
    ...
    for table_key, table in tables.items():
        ...
        existing = _extension_dict(table)
        unchanged = (
            str(existing.get(markdown_name, "")) == markdown
            and _reference_identity_set(existing.get(related_name))
            == _reference_identity_set(related_payload)
        )
        if unchanged:
