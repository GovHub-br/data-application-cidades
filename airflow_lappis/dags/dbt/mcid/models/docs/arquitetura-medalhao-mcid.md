...
| Série histórica multi-mês na Bronze | Reloginho (grupo A) **refatorado** em bronze → silver → gold em `indicadores_mcmv_dbt/` (ver `issue-130-refatoracao-medalhao-reloginho.md`); ainda materializado em DuckDB, promoção para Postgres pendente do ADR #117 |
