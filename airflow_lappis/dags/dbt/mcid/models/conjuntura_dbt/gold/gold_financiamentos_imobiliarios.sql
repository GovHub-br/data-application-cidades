...
dez25 AS (
    ...
    FROM pivotado WHERE data_referencia = DATE '2025-12-01'
),

nov25 AS (
    SELECT pf_concessoes, pf_taxa_juros, pf_inadimplencia, pj_concessoes, pj_taxa_juros, pj_inadimplencia, dt_ingest, dt_silver
    FROM pivotado WHERE data_referencia = DATE '2025-11-01'
),

dez24 AS (
    SELECT pf_concessoes, pf_taxa_juros, pf_inadimplencia, pj_concessoes, pj_taxa_juros, pj_inadimplencia, dt_ingest, dt_silver
    FROM pivotado WHERE data_referencia = DATE '2024-12-01'
),
...
acum_dez25 AS (
    SELECT
        ...
        MAX(dt_silver)      AS dt_silver
    FROM pivotado
    WHERE data_referencia BETWEEN DATE '2025-01-01' AND DATE '2025-12-01'
),
...
acum_dez24 AS (
    SELECT
        ...
        MAX(dt_silver)      AS dt_silver
    FROM pivotado
    WHERE data_referencia BETWEEN DATE '2024-01-01' AND DATE '2024-12-01'
),
